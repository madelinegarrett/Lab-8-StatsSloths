...
* I, Zandy Boone, analyzed the data to determine which cases had the most cause of a firearm being discharged in the case. I did this by filtering the data by contact basis to determine in each set of cases, how many firearms were discharged vs no firearm being discharged. I determined that the highest cases were a weapon/concealed weapon, warrant, and vehicle stoppage caes. I then created a bar graph to visually show the data with weapon discharges being on the x-axis and the bars being filled with the type of contact basis. I also added labels to graph to more clearly convey what the graph is visualising and to show what each axis represents.
